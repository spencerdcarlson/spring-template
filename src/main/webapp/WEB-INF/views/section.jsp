...
<h2 id="mainSection" data-sectionId="${section.sectionId}">${section.sectionName} </h2>
<a data-toggle="modal" href="#myModal" class="btn btn-primary">Show Reading</a>

<section class="modal hide fade" id="myModal">
	<button type="button" class="close" data-dismiss="modal">×</button>
	<a  href='resources/img/${res.resourceName}.JPG' style="padding: 5px 5px 5px 5px;" type="button" >open in new tab</a>
...
<c:forEach var="quiz" items="${children}" varStatus="quizCount">
	<div class="row-fluid">
		<div class="span12">
			<h2 class="sectionTitle" data-sectionId="${quizSectionId}">${quiz.sectionName}</h2>
						...
						<c:forEach var="option" items="${fn:split(question.questionOptions, '//')}" varStatus="optionCount">
							<c:choose>
								<c:when test="${optionCount.count % 2 == 0 }" >
									<input type="radio" class ="answer" name="answer${question.questionId}-${questionCount.count}" data-questionid="${question.questionId}" data-optionid="${groupId}" value="${option}" onClick="clikedAnswer(this.getAttribute('data-questionid'),this.getAttribute('data-optionid'),this.value);" /> ${option}
...
<section>
<button onClick="quizDone();" class="btn">Submit</button>
</section>
